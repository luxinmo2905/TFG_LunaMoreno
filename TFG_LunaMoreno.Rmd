...
# Función de predicción adaptada a tu modelo
latentes <- function(gender, apoe4, apoe44, app, persenil1, persenil2, EYO) {
      
      x <-  unlist(output[,"betax[1]"]) + unlist(output[,"betax[1]"])*EYO + unlist(output[,"betax[6]"])*apoe4 + unlist(output[,"betax[7]"])*apoe44 + unlist(output[,"betax[4]"])*app + unlist(output[,"betax[2]"])*persenil1 + unlist(output[,"betax[3]"])*persenil2 + unlist(output[,"betax[8]"])*gender
    
      y <- unlist(output[,"betay[1]"]) + unlist(output[,"betay[1]"])*EYO + unlist(output[,"betay[6]"])*apoe4 + unlist(output[,"betay[7]"])*apoe44 + unlist(output[,"betay[4]"])*app + unlist(output[,"betay[2]"])*persenil1 + unlist(output[,"betay[3]"])*persenil2 + unlist(output[,"betay[8]"])*gender
    
      z <- unlist(output[,"betaz[1]"]) + unlist(output[,"betaz[1]"])*EYO + unlist(output[,"betaz[6]"])*apoe4 + unlist(output[,"betaz[7]"])*apoe44 + unlist(output[,"betaz[4]"])*app + unlist(output[,"betaz[2]"])*persenil1 + unlist(output[,"betaz[3]"])*persenil2 + unlist(output[,"betaz[8]"])*gender

lat <- data.frame(mri_mu0 = x, mri_mu1 = y,mri_mu2 = z)

res <- c(apply(lat, MARGIN = 2, mean),apply(lat, MARGIN = 2, quantile,prob=0.025), apply(lat, MARGIN = 2, quantile,prob=0.975))
names(res) <- c(names(lat), paste(names(lat),"_low", sep=""), paste(names(lat),"_sup", sep=""))
res 
}
...
p <- ggplot() +
  ...
  geom_ribbon(data = evol_no_no_0, aes(x = EYO, ymin = mri_mu2_low, ymax = mri_mu2_sup), alpha = 0.2, fill = "blue") +
  geom_line(data = evol_no_no_0, aes(x = EYO, y = mri_mu2), color = "blue")+
  geom_ribbon(data = evol_no_per2_0, aes(x = EYO, ymin = mri_mu2_low, ymax = mri_mu2_sup), alpha = 0.2, fill = "pink") +
  geom_line(data = evol_no_per2_0, aes(x = EYO, y = mri_mu2), color = "pink")+
  geom_ribbon(data = evol_44_per2_0, aes(x = EYO, ymin = mri_mu2_low, ymax = mri_mu2_sup), alpha = 0.2, fill = "orange") +
  geom_line(data = evol_44_per2_0, aes(x = EYO, y = mri_mu2), color = "orange")
# Mostrar el gráfico
print(p)
```
...
p <- ggplot() +
  ...
  geom_point(data = df, aes(x = DIAN_EYO, y = mri_z_mu_1)) +
  labs(x = "Estimated years to onset") +
  geom_ribbon(data = evol_no_per1_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "blue") +
  geom_line(data = evol_no_per1_0, aes(x = EYO, y = mri_mu1), color = "blue")+
  geom_ribbon(data = evol_4_per1_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "pink") +
  geom_line(data = evol_4_per1_0, aes(x = EYO, y = mri_mu1), color = "pink")+
  geom_ribbon(data = evol_44_per1_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "orange") +
  geom_line(data = evol_44_per1_0, aes(x = EYO, y = mri_mu1), color = "orange")
# Mostrar el gráfico
print(p)
```
```{r}
p <- ggplot() +
  ...
  geom_ribbon(data = evol_no_per1_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "blue") +
  geom_line(data = evol_no_per1_0, aes(x = EYO, y = mri_mu1), color = "blue")+
  geom_ribbon(data = evol_no_per2_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "pink") +
  geom_line(data = evol_no_per2_0, aes(x = EYO, y = mri_mu1), color = "pink")+
  geom_ribbon(data = evol_no_app_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "orange") +
  geom_line(data = evol_no_app_0, aes(x = EYO, y = mri_mu1), color = "orange")
# Mostrar el gráfico
print(p)
...
df_filtered <- df %>% 
  filter(Mutation2 == "Mutation", gender== 0)

# Definir límites de los ejes fuera de ggplot
x_limits <- range(df$DIAN_EYO, na.rm = TRUE)
y_limits <- range(df$mri_z_mu_0, na.rm = TRUE)

# Creación del gráfico
p <- ggplot() +
  geom_line(data = df_filtered, aes(x = DIAN_EYO, y = mri_z_mu_1, group = id)) +
  geom_point(data = df_filtered, aes(x = DIAN_EYO, y = mri_z_mu_1)) +
  labs(x = "Estimated years to onset") +
  ylim(y_limits) +
  xlim(x_limits) +
  geom_ribbon(data = evol_4_no_0, aes(x = EYO, ymin = mri_mu1_low, ymax = mri_mu1_sup), alpha = 0.2, fill = "blue") +
  geom_line(data = evol_4_no_0, aes(x = EYO, y = mri_mu1), color = "blue")
