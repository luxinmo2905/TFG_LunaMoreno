...
#variables control geneero y variable cognitiva general y ver que papel juegan la mutacion y el apoe en la evolucion de estas variables latentes

#solo mutation solo apoe, solo suma e interacción

#la tabla la dejamios dinal dicendoq ue se han probado más modelos que no hemos elegido y estos modelos tabla dic pequeña en el texto siempre considerando interacción son los cambios temporal de apoe y mutación

#primero hablar sobre modelo, comentar pruebas de variables para ajustar variables y finalmente quedarnos con las que tienen algo que consideramos relevante para loq ue queremos estudiar

#en base a eso hablamos de convergencia, comentar crterios, mezcla cadenas, shrink factor y autocorrelación. entre 0 y 0.1 consideramos modleo converge

#graficas modelo evolución
...
latentes <- function(gender, apoe4, apoe44, app, presenil1, presenil2, EYO) {
  
  x <- unlist(output[,"betax[1]"]) + 
       unlist(output[,"betax[5]"])*EYO + 
       unlist(output[,"betax[6]"])*apoe4 + 
       unlist(output[,"betax[7]"])*apoe44 + 
       unlist(output[,"betax[10]"])*EYO*apoe4 + 
       unlist(output[,"betax[11]"])*EYO*apoe44 + 
       unlist(output[,"betax[4]"])*app + 
       unlist(output[,"betax[2]"])*presenil1 + 
       unlist(output[,"betax[3]"])*presenil2 + 
       unlist(output[,"betax[8]"])*gender + 
       unlist(output[,"betax[9]"])

  y <- unlist(output[,"betay[1]"]) + 
       unlist(output[,"betay[5]"])*EYO + 
       unlist(output[,"betay[6]"])*apoe4 + 
       unlist(output[,"betay[7]"])*apoe44 + 
       unlist(output[,"betay[10]"])*EYO*apoe4 + 
       unlist(output[,"betay[11]"])*EYO*apoe44 + 
       unlist(output[,"betay[4]"])*app + 
       unlist(output[,"betay[2]"])*presenil1 + 
       unlist(output[,"betay[3]"])*presenil2 + 
       unlist(output[,"betay[8]"])*gender + 
       unlist(output[,"betay[9]"])

  z <- unlist(output[,"betaz[1]"]) + 
       unlist(output[,"betaz[5]"])*EYO + 
       unlist(output[,"betaz[6]"])*apoe4 + 
       unlist(output[,"betaz[7]"])*apoe44 + 
       unlist(output[,"betaz[10]"])*EYO*apoe4 + 
       unlist(output[,"betaz[11]"])*EYO*apoe44 + 
       unlist(output[,"betaz[4]"])*app + 
       unlist(output[,"betaz[2]"])*presenil1 + 
       unlist(output[,"betaz[3]"])*presenil2 + 
       unlist(output[,"betaz[8]"])*gender + 
       unlist(output[,"betaz[9]"])
...
```{r}
simular_predictor_lineal_jags <- function(output, variable_prefix = "betax", new_data_vector, colnames_X) {
  # output: resultado de coda.samples (objeto mcmc.list)
  # variable_prefix: "betax", "betay" o "betaz"
  # new_data_vector: vector numérico con los valores de la fila de diseño
  # colnames_X: nombres de las columnas de la matriz de diseño usada en JAGS (colnames(X))
  
  # Convertimos output a matriz si no lo es
  samples_mat <- as.matrix(output)
  
  # Extraemos las columnas relevantes de beta
  beta_cols <- grep(paste0("^", variable_prefix, "\\["), colnames(samples_mat), value = TRUE)
  
  # Ordenamos por el índice dentro de betax[]
  beta_ordered <- beta_cols[order(as.numeric(gsub(".*\\[|\\]", "", beta_cols)))]
  
  beta_mat <- samples_mat[, beta_ordered]
  
  if (length(new_data_vector) != ncol(beta_mat)) {
    stop("La longitud del vector de predictores no coincide con la de beta.")
  }
  
  # Calculamos el predictor lineal para cada simulación posterior
  pred_lineal <- as.numeric(beta_mat %*% new_data_vector)
